...
class VotingTestCase(BaseTestCase):
    ...
    def test_question_yes_no_options(self):
        ...
        #login with user admin
        self.login()
        print(self.voting.question.cattegory)
        self.voting.create_pubkey()
        self.voting.save()
        
        print(self.voting.question.options.all)
        
        data = {'action': 'start'}
        response = self.client.put('/voting/{}/'.format(self.voting.pk), data, format='json')
        # Verifica que la respuesta sea HTTP 200 OK y el mensaje sea 'Voting started'
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data, 'Voting started')
        
        options = self.voting.question.options
        print(options) 
...
#         response = self.client.put('/voting/{}/'.format(voting.pk), data, format='json')
#         self.assertEqual(response.status_code, 400)
#         self.assself.assertEqual(response.status_code, 400)ertEqual(response.json(), 'Voting already stopped')
